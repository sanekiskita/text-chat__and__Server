 ...
 */
    onSendMessage=()=>{
        ...
        this.props.message.push({'userName':this.props.userName,'text':this.state.valueMessage})
    }
    ...
    render(){
        return (
            ...
            {this.props.message.map((message,index)=>(
                 <div key={index} className={`Message  ${message.userName!==this.props.userName?'':'My'}`}><div className="Message__Text"><p>{message.text}</p><p className="Message__Auth">автор: {message.userName}</p></div> </div>
            ))}
            </div>
            </div>
            <div className="Container__Input">
              <input  className="Message__input"  type="text" placeholder="Введите текст" value={this.state.valueMessage} onKeyPress={this.handleKeyPress} onChange={e=>{this.setState({valueMessage:e.target.value});}}  ></input>
