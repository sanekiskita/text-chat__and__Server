    ...
    render(){
        return (
            ...
            {this.props.message.map((message,index)=>(
                 <div key={index} className={`Message  ${message.userName!==this.props.userName?'':'My'}`}><div className="Message__Text"><p>{message.text}</p><p className="Message__Auth">{message.date}  автор: {message.userName}</p></div> </div>
            ))}
            </div>
            </div>
